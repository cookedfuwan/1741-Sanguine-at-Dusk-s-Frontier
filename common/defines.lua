...
country = {
	...
	BADBOY_LIMIT = 25,
	MAX_BUREAUCRACY_PERCENTAGE = 0.02, -- More than max percent bureaucrats of poplation will give no additional benefits
	...
	MIN_CRIMEFIGHT_PERCENT = 0.05,
	MAX_CRIMEFIGHT_PERCENT = 0.99,
	ADMIN_EFFICIENCY_CRIMEFIGHT_PERCENT = 0.30, -- Crimefight depends on both state admin eff. and admin spending, admin spending percent effect is set to (1-ADMIN_EFFICIENCY_CRIMEFIGHT_PERCENT)
	CONSERVATIVE_INCREASE_AFTER_REFORM = 0.35, -- how many more conservatives in a upper house.
	...
	COLONIAL_RANK = 15, -- Minimum rank a nation must have to send colonists
	COLONY_TO_STATE_PRESTIGE_GAIN = 6, -- Prestige gain when turning colony to state
	COLONIAL_LIFERATING = 35,
	BASE_GREATPOWER_DAILY_INFLUENCE = 0.25, -- Influence value which is distributed each day
	AI_SUPPORT_REFORM = 0.07, -- At least this many % needs to support a reform for the AI to take it
	BASE_MONTHLY_DIPLOPOINTS = 0.15, -- Base value gain for diplomatic actions each month
	DIPLOMAT_TRAVEL_TIME = 45,
	...
	MOVEMENT_RADICALISM_PASSED_REFORM_EFFECT  = -3,
	MOVEMENT_RADICALISM_NATIONALISM_FACTOR = 1.0,
	SUPPRESSION_POINTS_GAIN_BASE = 200, -- monthly gain with max bureaucrats
	...
	INVESTMENT_SCORE_FACTOR = 0.0075, -- how much foreign investment money counts towards your industry score
	UNCIV_TECH_SPREAD_MAX = 0.1,
	UNCIV_TECH_SPREAD_MIN = 0.01,
	MIN_DELAY_BETWEEN_REFORMS = 8, -- months
	...
	COLONIZATION_MONTHS_TO_COLONIZE = 36,
	COLONIZATION_DAYS_BETWEEN_INVESTMENT = 180,
	COLONIZATION_DAYS_FOR_INITIAL_INVESTMENT = 240,
	...
	COLONIZATION_CREATE_COLONY_COST = 5, -- per province
	...
	PARTY_LOYALTY_HIT_ON_WAR_LOSS = 0.9, -- Drops the ruling party loyalty if war is lost.
	RESEARCH_POINTS_ON_CONQUER_MULT = 140, -- multiplier to RP got by conquering as unciv when got enacted military reforms
	MAX_RESEARCH_POINTS = 25000, -- max RP you can store for uncivs
},

economy = {
	MAX_DAILY_RESEARCH		    = 90,
	...
	GOODS_FOCUS_SWAP_CHANCE = 40, -- Percent increased chance that artisan wants to change goods independantly of how well he is doing presently
	...
	RGO_SUPPLY_DEMAND_FACTOR_HIRE_LO = 0.05,	-- how fast pops are Hired when there is a medium demand
...
military = {
	...
	COMBATLOSS_WAR_EXHAUSTION = 0.65, -- base war exhastion in combat
	...
	NAVAL_COMBAT_MAX_TARGETS = 4, -- max number of ships that may target the same enemy ship
	...
	AI_CAVALRY_PROPORTION = 0.25, -- fraction of brigades that should be cavalry
	AI_SUPPORT_PROPORTION = 0.25, -- fraction of brigades that should be artillery and other support units
	...
	AI_BLOCKADE_RANGE = 250, -- max distance the AI will send out blockade fleets from their home base
	...
	RECON_SIEGE_EFFECT = 0.65, -- multiplier to effect of recon on speeding up sieges
...
diplomacy = {
	...
	INFAMY_STATUS_QUO = 0,
	INFAMY_ANNEX = 15,
	INFAMY_DEMAND_STATE = 10,
	INFAMY_INSTALL_COMMUNIST_GOV_TYPE = 5,
	INFAMY_UNINSTALL_COMMUNIST_GOV_TYPE = 5,
	INFAMY_COLONY = 5,
	
	PRESTIGE_ADD_TO_SPHERE_BASE = 2,
	PRESTIGE_RELEASE_PUPPET_BASE = 10,
	PRESTIGE_MAKE_PUPPET_BASE = 4,
	...
	GOOD_PEACE_REFUSAL_WAREXH = 2.5, --War exhaustion hit from refusing a good peace offer
	...
	GW_INTERVENE_MAX_EXHAUSTION = 10, -- max war exhaustion required to intervene in great war
	...
	TWS_FULFILLED_SPEED = 0.01, -- Ticking War Score grows up with this speed daily once CB is fulfilled
	TWS_NOT_FULFILLED_SPEED = 0.01, -- Ticking War Score falls down when CB is not fulfilled (or after grace period)
	TWS_GRACE_PERIOD_DAYS = 730, -- Ticking War Score delay before it starts falling down for not fulfilling CB.
	TWS_CB_LIMIT_DEFAULT = 125,
	TWS_FULFILLED_IDLE_SPACE = 0.4, -- How much % the CB fulfillment must done, so TWS starts ticking.
	TWS_BATTLE_MIN_COUNT = 3, -- At least X battles before the aspect of wins will count
	TWS_BATTLE_MAX_ASPECT = 60.0, -- Max allowed battle wins aspect for TWS
...
pops = {
	BASE_CLERGY_FOR_LITERACY = 0.0025, 
	MAX_CLERGY_FOR_LITERACY = 0.009, 
	...
	BASE_GOODS_DEMAND = 0.375,
	BASE_POPGROWTH = 0.00001,
	MIN_LIFE_RATING_FOR_GROWTH = 25,
	LIFE_RATING_GROWTH_BONUS = 0.00005,
	LIFE_NEED_STARVATION_LIMIT = 0.35,

	MIL_LACK_EVERYDAY_NEED = 0.001,
	MIL_HAS_EVERYDAY_NEED = -0.025,
	MIL_HAS_LUXURY_NEED = -0.01,
	MIL_NO_LIFE_NEED = 0.002,
	MIL_REQUIRE_REFORM = 0.2,
	MIL_IDEOLOGY = -0.1,
	MIL_RULING_PARTY = -0.1,
	MIL_REFORM_IMPACT = 2,
	MIL_WAR_EXHAUSTION = 0.001,
	...
	REDUCTION_AFTER_RISEING = 5.0, -- After a pop spawns a rebellion, its militancy will be reduced this much
	REDUCTION_AFTER_DEFEAT = 12.0, -- After a rebellion is being defeated in combat, its pop militancy will be divided by this number.
	...
	ARTISAN_MIN_PRODUCTIVITY = 10, -- Minimum efficieny of an artisan
	...
	LUXURY_CON_CHANGE = 0.002, -- con boost from over-buying luxuary goods
	...
	REBEL_OCCUPATION_STRENGTH_BONUS = 0.002, -- the amount of strength given to rebel movements when they occupy a province
...
ai =
{
	COLONY_WEIGHT = 0.0015, -- ai weight for colonising
	...
	SOLDIER_FRACTION = 0.02, -- max amount of population AI wants to be soldiers
	...
	SPAM_PENALTY = 5, -- makes certain diplomatic action less common 
	ONE_SIDE_MAX_WARSCORE = 200, -- don't add too many wargoals to one side in a war
	POP_PROJECT_INVESTMENT_MAX_BUDGET_FACTOR = 0.4, -- how much % of our current budget can be spend on the pop project investments.
	...
	TWS_AWARENESS_SCORE_ASPECT = 0.55, -- AI will not add any more CBs when TWS is more then X% of total WS. (to not destroy the progress)
	...
	PEACE_WAR_EXHAUSTION_FACTOR = 0.75, -- AI willingness to peace based on war exhaustion
	...
	AGGRESSION_UNCIV_BONUS = 55, -- additional AI civ aggression against uncivs
	...
	MONTHS_BEFORE_DISBAND = 48, -- Months from start date before AI will disband armies/navies (to avoid disbands because of early economic turmoil)
},
